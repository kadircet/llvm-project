...
- clang-tidy and include cleaner diagnostics has links to their documentation.
...
- Hover displays AKA types by default now.
...
- Support for symbols exported with using-decls in all features.
- Support for concepts and requires in all features.
- Fixed background-indexing priority for M1 chips.
- Indexing for standard library symbols.
- Obj-C framework includes are spelled properly during include insertion operations.
